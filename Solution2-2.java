...
class Solution2 {
	...
	static void makeSet(Set subset,int x){
		subset.parent=x;
		subset.rank=0;
	}
	...
	public static int kruskal() {
		...
		while (cnt < N - 1) {
			Node node = edge[c];
			c++;
			int x = find(subsets, node.start - 1);
			int y = find(subsets, node.end - 1);
			if (x != y) {
				sum += node.weight;
				Union(subsets, x, y);
				cnt++;
	...
	static void quickSort(Node[] data, int start, int end){
		if(start >= end){
			return;
		}
		int pivot = start;
		...
		int j = end;
		while(i <= j){
			while(i <= end && data[i].weight >= data[pivot].weight){
				i++;
			}
			while(j > start && data[j].weight <= data[pivot].weight){
				j--;
			}
			if(i > j){
				swap(data,j,pivot);
	...
	static void swap(Node[] data,int i,int j){
		Node temp=data[i];
		data[i] = data[j];
		data[j] = temp;
	}
